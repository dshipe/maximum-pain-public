...
namespace MaxPainInfrastructure.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<string> ScreenerGenerateAndSendOverride(bool debug, bool useShortUrls, bool runNow)
        {
            ...
            string html = await ScreenerGenerateAndSend(xmlSettings, imageTicker, Convert.ToBoolean(debug), Convert.ToBoolean(useShortUrls), string.Empty, militaryHour);
            await _awsContext.SettingsPost(xmlSettings.OuterXml);

            return html;
        }

        public async Task<string> ScreenerGenerateAndSend(XmlDocument xmlSettings, string imageTicker, bool debug, bool useShortUrls, string message, int militaryHour)
        {
            DateTime current = DateTime.UtcNow;
            if (militaryHour != -1)
            {
