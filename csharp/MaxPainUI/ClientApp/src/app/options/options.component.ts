...
export class OptionsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.tickerObj.Ticker=="AIRBUS") this.redirect("not-found", "");
		
    if (this.tickerObj.Ticker=="OPEN-INTEREST"
      || this.tickerObj.Ticker=="VOLUME"
      || this.tickerObj.Ticker=="MAX-PAIN"
      || this.tickerObj.Ticker=="SPREADS"
      || this.tickerObj.Ticker=="POORMANS-ALGO"
      || this.tickerObj.Ticker=="HIGH-OPEN-INTEREST"
      || this.tickerObj.Ticker=="DATA"
    )
    {
  ...
  loadMaxPain(): boolean  {
    ...
    if (this.chain.straddles.length==0) return false;
    if (!this.tickerObj.JsonData) return false;
    if (!this.tickerObj.MaturityString) return false;

    this.logTickerObj("loadMaxPain");
    
    let observable$: Observable<MPChn> = 
      this.data.getMaxpainPost(this.tickerObj.JsonData, this.tickerObj.MaturityString);
    observable$.subscribe(response => {
      this.mpChain = response;
    });
    return true;
  }
